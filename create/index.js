...
const templates = {
  components: {
    default: './create/templates/components/pexample.tsx',
    typings: './create/templates/components/pexample.typings.tsx'
  },
  ...
  modules: {
    ...
    graphql: [
      './create/templates/modules/graphql/exampleGraphql.tsx',
      './create/templates/modules/graphql/index.tsx'
    ],
...
const createFolder = (type, folder) => {
  ...
  switch(type) {
    case 'components': 
      path = `./src/${type}/${folder}/${camelCase(`p-${name}`, { pascalCase: true })}`
...
const createComponent = {
  default: folderName => {
    ...
    const file = `p${camelCase(name)}.tsx`
  ...
  typings: folderName => {
    const folder = `${createFolder('components', folderName)}/`
    const file = `p${camelCase(name)}.typings.tsx`
    const path = folder + file
...
const createModule = {
  ...
  graphql: () => {
    const folder = `${createFolder('modules', 'graphql')}/`
    const file = [
      `${camelCase(name)}Graphql.tsx`,
      'index.tsx'
    ]
    ...
    if (!checkPath(path)) {
      ...
      for (const index in templates.modules.graphql) {
        shell.exec(`cat ${templates.modules.graphql[index]} > ${path[index]}`)
